...
## 强制输出契约（vault-data-warehouse-creation）— 分享控制级别必须标注

任何 vault 创建、初始化、或修改输出**必须**显式标注 4 级分享控制（`sharing-settings.json`）的当前状态，**4 个级别全部列出 + 当前选中级别用 ✅ 标注 + 每级附带具体含义**。模糊措辞（"分享控制已设置"/"已配置隐私"）= bug。

```
═══════════════════════════════════════════
📊 当前分享控制级别（sharing-settings.json）
═══════════════════════════════════════════

[ ✅ ] 🔒 private          — 仅本人可读；任何外部访问会被 access-log 拒绝并记录
[    ] 🔑 authorized       — 邮箱/ID 白名单授权；当前授权列表：<空 / 列表>
[    ] 📊 anonymized-for-AI — 脱敏后供 AI 训练/医学研究；脱敏规则：去姓名、出生日期精度到年、地域精度到省
[    ] 🌐 public           — 完全匿名公开贡献；仅保留临床特征+基因变异+治疗应答

⚠️ 初始化默认 = 🔒 private（永远不默认 public）
⚠️ 一旦切换到 🌐 public 不可撤回——需二次输入 "I-UNDERSTAND-PUBLIC-IS-IRREVERSIBLE" 确认
...
- 国内估计患者数：<X 人>
- 风险级别：🔴 高（百人量级或以下）/ 🟡 中 / 🟢 低
- 触发条件：罕见病 + 基因变异组合在百万级数据集里通常只有几十人 → 即使去姓名也可被重识别
- 家系数据警告：<如有家系数据，提示"共享一人 = 暴露整个家族遗传信息"，否则写 "无家系数据"〉
```

**硬规则**：
- 这个块必须包含**全部 4 个级别**——不能只显示当前级别。
- 当前级别用 `[ ✅ ]` 显式标注，未选用 `[    ]`。
- 每级**必须**附带一句话含义，不接受"参见文档"。
- 重识别风险评估**与**分享控制是一对——只要分享级别变化，风险评估必须重新输出。
- 切换到 `🌐 public` 必须二次确认（输入特定字符串），不能"用户说 OK 就 OK"。

为什么这条存在：2026-05 agent-skills-eval 显示 vault-data-warehouse-creation 用例在没有显式 4 级标注要求时，模型会笼统说"我帮你设置好了隐私"，但用户既不知道当前是哪级、也不知道有几级可选。罕见病数据重识别风险极高，分享控制必须可视化。
